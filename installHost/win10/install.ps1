...
if($hyperv.State -eq "Enabled") {
    ...
    if($wsl.State -eq "Enabled") {
        ...
        if (-not (Test-Path nomachine.exe))
        {
            Write-Host "Installing NoMachine."

            Write-Host "Downloading the NoMachine exe"
            Invoke-WebRequest -Uri https://download.nomachine.com/download/8.10/Windows/nomachine_8.10.1_1_x64.exe -OutFile nomachine.exe -UseBasicParsing
            Write-Host "Download Completed"
            Write-Host "Installing NoMachine..."
            start-process .\nomachine.exe "/verysilent" -Wait -NoNewWindow
